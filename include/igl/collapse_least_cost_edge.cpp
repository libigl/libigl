...
IGL_INLINE bool igl::collapse_least_cost_edge3(
    ...
    int& f2)
{
    ...
    while (true)
    {
        ...
        if (std::get<2>(p) == EQ(e))
        {
            // std::cout << "cost: " << std::get<0>(p) << std::endl;
            break;
        }
        // must be stale or dead.
        assert(std::get<2>(p) < EQ(e) || EQ(e) == -1);
    ...
    //����µ��Q����
    int source_id = E(e, 0), target_id = E(e, 1);
    Eigen::Matrix4d new_q = q_matrices[source_id] + q_matrices[target_id];

    //�޸�ǰ��
    //bool collapsed = true;
    //if (pre_collapse(V, F, E, EMAP, EF, EI,PD2, Q, EQ, C, e))
    //{
    //    collapsed = collapse_edge(
    //        e, C.row(e),
    //        Nsv, Nsf, Ndv, Ndf,
    ...
    //    collapsed = false;
    //}
    //post_collapse(V, F, E, EMAP, EF, EI, PD2, Q, EQ, C, e, e1, e2, f1, f2, collapsed);
    ...
    if (collapsed)
    {
		...
		for (auto& tmp : Nf)
		{
			for (int i = 0;i < 3;i++)
			{
				ver.push_back(F(tmp, i));
			}
		}
        std::sort(ver.begin(), ver.end());
        ver.erase(std::unique(ver.begin(), ver.end()), ver.end());
        q_matrices[source_id] = new_q;
        q_matrices[target_id] = new_q;
        Eigen::MatrixXd PD3, PD4;
        Eigen::VectorXd PV3, PV4;

        igl::principal_curvature(V, F, PD3, PD4, PV3, PV4, ver);
		for (int i = 0;i < ver.size();i++)
		{
            /*std::cout << ver[i] << " " << PD2.row(ver[i]) << std::endl;
            std::cout << ver[i] << " " << PV2(ver[i]) << std::endl;*/
			PD2.row(ver[i]) = PD4.row(ver[i]);
			PV2(ver[i]) = PV4(ver[i]);
            /*std::cout << ver[i] << " " << PD4.row(ver[i]) << std::endl;
			std::cout << ver[i] << " " << PV4(ver[i]) << std::endl;*/
		}
        // Collect all edges that must be updated
        std::vector<int> Ne;
        Ne.reserve(3 * Nf.size());
        ...
        for (auto& ei : Ne)
        {
            ...
            double cost;
            Eigen::RowVectorXd place;
            cost_and_placement(ei, V, F, E, EMAP, EF, EI, PD2,PV2, q_matrices, avg_edges_lenth, cost, place);
            // Increment timestamp
            EQ(ei)++;
            // Replace in queue
            Q.emplace(cost, ei, EQ(ei));
