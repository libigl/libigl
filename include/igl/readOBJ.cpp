  ...
  std::string &usemtl)
{
  ...
  std::string ml("mtllib");
  std::string um("usemtl");
#ifndef IGL_LINE_MAX
  ...
  while (fgets(line, IGL_LINE_MAX, obj_file) != NULL)
  {
    ...
    if(sscanf(line, "%s",type) == 1)
    {
      ...
      }else if(type == ml)
      {
        ...
        {
          fprintf(stderr,
                  "Error: readOBJ() on line %d wrong mtllib parameter",
                  line_no);
          fclose(obj_file);
          return false;
        }
    ...
    std::string &usemtl)
{
  ...
  bool success = igl::readOBJ(str,vV,vTC,vN,vF,vFTC,vFN, mtllib, usemtl);
  if(!success)
  {
