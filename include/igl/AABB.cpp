...
#else
  // DFS
  const Scalar min_t = 0;
  return intersect_ray(
    V,Ele,origin,dir,min_t,hit);
#endif
}
  ...
  igl::Hit<typename DerivedV::Scalar> & hit) const
{
  ...
  RowVectorDIMS inv_dir_pad = inv_dir;
  igl::increment_ulp(inv_dir_pad, 2);
  const Scalar max_t = std::numeric_limits<Scalar>::infinity();
  return intersect_ray_opt(V, Ele, origin, dir, inv_dir, inv_dir_pad, _min_t, max_t, hit);
}
...
igl::AABB<DerivedV,DIM>::intersect_ray_opt(
  ...
  igl::Hit<typename DerivedV::Scalar> & hit) const
{
  // Set user min_t for box intersection test
  const Scalar min_t = _min_t;
  const Scalar t0 = _min_t;
  // Carry recursive max_t for box intersection test
  // since we're only searching for the first hit, min(t), in this case
  Scalar max_t = _max_t;
  const Scalar t1 = _max_t;
  {
    Scalar _1,_2;
    if(!ray_box_intersect(origin,inv_dir,inv_dir_pad,m_box,t0,t1,_1,_2))
    {
  ...
  // nmlucarelli: This might make a difference now with the min_t/max_t patch
  ...
  bool left_ret = m_left->intersect_ray_opt(V,Ele,origin,dir,inv_dir,inv_dir_pad,min_t,max_t,left_hit);
  if(left_ret && left_hit.t<max_t)
  {
    // nmlucarelli: min_t/max_t patch should fix this behavior
  ...
  bool right_ret = m_right->intersect_ray_opt(V,Ele,origin,dir,inv_dir,inv_dir_pad,min_t,max_t,right_hit);
  if(right_ret && right_hit.t<max_t)
  {
    max_t = right_hit.t;
    hit = right_hit;
    right_ret = true;
...
template bool igl::AABB<Eigen::Matrix<double, -1, -1, 0, -1, -1>, 3>::intersect_ray
