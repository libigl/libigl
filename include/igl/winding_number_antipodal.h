...
namespace igl
{
  ...
  public:
    ...
    {
      assert(V.cols() == 3 && "AntipodalWindingNumberScene: only 3D vertex positions are supported");
      assert(F.cols() == 3 && "AntipodalWindingNumberScene: only triangle meshes are supported");
      ...
      std::vector<WeightedSeg> & out)
    {
      ...
      counts.reserve(static_cast<size_t>(F.rows()) * 2);

      const auto pack = [](int lo, int hi) -> uint64_t
      {
      ...
      for (const auto & kv : counts)
      {
        ...
        if (sum == 0) continue;
        const uint32_t lo = static_cast<uint32_t>(kv.first >> 32);
        const uint32_t hi = static_cast<uint32_t>(kv.first & 0xFFFFFFFFu);
  ...
  /// One-shot generalized winding number via the Antipodal Method:
  /// builds a `WindingNumberAntipodalScene<Scalar>` (with `Scalar` deduced from `DerivedV`) 
  /// and runs the batch query against the supplied `Intersector`.
  ///
  ...
  ///                          `igl::embree::EmbreeIntersector` satisfies the concept and works out of the box. 
  ///                          Just call its `init(V.cast<float>(), F.cast<int>())`.
  /// @param[in]  O            #O by 3 list of query points
  /// @param[out] W            #O by 1 list of winding numbers
  ///
    ...
    Eigen::PlainObjectBase<DerivedW> & W)
  {
    ...
    WindingNumberAntipodalScene<Scalar> scene(V, F);
