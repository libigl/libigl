...
namespace igl
{
  ...
  /// \see PlainMatrix
  template <typename Derived, int Size = (Derived::ColsAtCompileTime == 1 && Derived::RowsAtCompileTime != 1) ? Derived::RowsAtCompileTime : Derived::ColsAtCompileTime>
  using PlainVector = typename PlainVectorHelper<Derived, Size>::Type;
