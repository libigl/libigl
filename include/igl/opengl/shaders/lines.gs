...
void corner(vec2 coord)
{
  ...
  frag.ex = normalize(cross(frag.ez, frag.ey));
  ...
  vec3 corner_position_eye = mix(vert[0].point_position_eye, vert[1].point_position_eye, 0.5*coord.t+0.5);
  corner_position_eye += coord.s * max(vert[0].point_radius, vert[1].point_radius) * frag.ex;
  // vec4 corner_position_eye = vec4(vert[0].point_position_eye, 1.0);
  // corner_position_eye.xy += vert[0].point_radius * coord * BOX_CORRECTION;
