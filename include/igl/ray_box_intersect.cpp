...
#else
  Scalar txmin, txmax;
  txmin = (bounds[sign[0]](0)-origin(0)) * inv_dir(0);
  txmax = (bounds[1-sign[0]](0)-origin(0)) * inv_dir(0);
  tymin = (bounds[ sign[1]](1)-origin(1)) * inv_dir(1);
  tymax = (bounds[1-sign[1]](1)-origin(1)) * inv_dir(1);
  tzmin = (bounds[ sign[2]](2)-origin(2)) * inv_dir(2);
  tzmax = (bounds[1-sign[2]](2)-origin(2)) * inv_dir(2);
  tmin = berger_perrin_max(tzmin, berger_perrin_max(tymin, berger_perrin_max(txmin, t0)));
  tmax = berger_perrin_min(tzmax, berger_perrin_min(tymax, berger_perrin_min(txmax, t1)));
  //  if ((not (tmin <= tmax)) and (tmin <= tmax *1.00000024f))
      //std::cout << std::setprecision(16) << "t0 = " << t0 << " t1 = " << t1 << " tmin = " << tmin << " tmax = " << tmax << " txmin = " << txmin << " txmax = " << txmax << " tymin = " << tymin << " tymax = " << tymax << " tzmin = " << tzmin << " tzmax = " << tzmax << " orig = " << origin << " dir = " << dir << std::endl;
