...
namespace embree
{
  ...
  void BVH4Intersector1<TriangleIntersector>::intersect(const BVH4Intersector1* This, Ray& ray)
  {
    AVX_ZERO_UPPER();
    ...
    while (true) pop:
    {
      ...
      while (true)
      {
        ...
        const ssef tNearX = msub(ssef((const char*)node+nearX), rdir.x, org_rdir.x);
        const ssef tNearY = msub(ssef((const char*)node+nearY), rdir.y, org_rdir.y);
        const ssef tNearZ = msub(ssef((const char*)node+nearZ), rdir.z, org_rdir.z);
        const ssef tFarX  = msub(ssef((const char*)node+farX ), rdir.x, org_rdir.x);
        const ssef tFarY  = msub(ssef((const char*)node+farY ), rdir.y, org_rdir.y);
        const ssef tFarZ  = msub(ssef((const char*)node+farZ ), rdir.z, org_rdir.z);
#else
  ...
  template<typename TriangleIntersector>
  bool BVH4Intersector1<TriangleIntersector>::occluded(const BVH4Intersector1* This, Ray& ray)
  {
    AVX_ZERO_UPPER();
	...
	size_t mask = movemask(tNear <= tFar);
        ...
        *stackPtr = c1; stackPtr++;
        ...
        r = __bsf(mask); mask = __btc(mask,r); cur = node->child(r); 
  ...
  {
    TriangleMesh::intersectors1.add("bvh4","triangle1i","fast","moeller" ,false,BVH4Intersector1<Triangle1iIntersector1<Intersector1MoellerTrumbore> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle1i","fast","pluecker",true ,BVH4Intersector1<Triangle1iIntersector1<Intersector1Pluecker> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle1v","fast","moeller" ,false,BVH4Intersector1<Triangle1vIntersector1<Intersector1MoellerTrumbore> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle1v","fast","pluecker",true ,BVH4Intersector1<Triangle1vIntersector1<Intersector1Pluecker> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle4i","fast","moeller" ,false,BVH4Intersector1<Triangle4iIntersector1<Intersector1MoellerTrumbore> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle4i","fast","pluecker",true ,BVH4Intersector1<Triangle4iIntersector1<Intersector1Pluecker> >::create);
    TriangleMesh::intersectors1.add("bvh4","triangle4v","fast","moeller" ,false,BVH4Intersector1<Triangle4vIntersector1<Intersector1MoellerTrumbore> >::create);
...
#endif
    TriangleMesh::intersectors1.setAccelDefaultTraverser("bvh4","fast");

    VirtualScene::intersectors1.add("bvh4","virtual"   ,"fast","virtual" ,true ,BVH4Intersector1<VirtualObjectIntersector1>::create);
    VirtualScene::intersectors1.setAccelDefaultTraverser("bvh4","fast");
  }
}
