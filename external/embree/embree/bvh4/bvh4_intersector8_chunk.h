...
namespace embree
{
  ...
  template<typename TriangleIntersector>
    class BVH4Intersector8Chunk : public Intersector8
  {
  ...
  public:
    BVH4Intersector8Chunk (const BVH4* bvh) 
      : Intersector8((intersectFunc)intersect,(occludedFunc)occluded), bvh(bvh) {}

    static Intersector8* create(const Accel* bvh) { 
      return new BVH4Intersector8Chunk((const BVH4*)bvh); 
    }

    static void   intersect(const BVH4Intersector8Chunk* This, Ray8& ray, const __m256 valid);
    static __m256 occluded (const BVH4Intersector8Chunk* This, Ray8& ray, const __m256 valid);
