...
namespace embree
{
  ...
  __forceinline Sample3f uniformSampleSphere(const float& u, const float& v) {
    ...
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), float(one_over_four_pi));
  }
  ...
  __forceinline Sample3f cosineSampleSphere(const float& u, const float& v) {
    ...
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), 2.0f*cosTheta*float(one_over_pi));
  }
  ...
  __forceinline Sample3f uniformSampleHemisphere(const float& u, const float& v) {
    ...
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), float(one_over_two_pi));
  }
  ...
  __forceinline Sample3f cosineSampleHemisphere(const float& u, const float& v) {
    ...
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), cosTheta*float(one_over_pi));
  }
  ...
  __forceinline Sample3f powerCosineSampleHemisphere(const float& u, const float& v, float exp) {
    ...
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), (exp+1.0f)*pow(cosTheta,exp)*float(one_over_two_pi));
  }
  ...
  __forceinline Sample3f uniformSampleCone(const float& u, const float& v, const float& angle) {
    ...
    const float sinTheta = cos2sin(cosTheta);
    return Sample3f(Vec3f(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta), rcp(float(four_pi)*sqr(sin(0.5f*angle))));
  }
