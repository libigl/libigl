...
namespace embree
{
  ...
  public:
    ...
    __forceinline FresnelConstant(Col3f c) : c(c) {}

    /*! Evaluation simply returns constant. */
    __forceinline Col3f eval(float cosTheta) const { return c; }

  private:
    Col3f c;  //!< constant reflectivity
  };
  ...
  public:
    ...
    __forceinline Col3f eval(float cosTheta) const {
      ...
      return Col3f(f0+(1.0f-f0)*b*b*b*b*b);
    }
  ...
  private:
    const Col3f eta;  //!< Real part of refraction index
    const Col3f k;    //!< Imaginary part of refraction index
  };
}
