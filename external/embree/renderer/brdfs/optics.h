...
namespace embree
{
  ...
  __forceinline Col3f fresnelConductor(float cosi, const Col3f& eta, const Col3f& k)
  {
    Col3f tmp = eta*eta + k*k;
    Col3f Rpar = (tmp * cosi*cosi - 2.0f*eta*cosi + Col3f(one)) *
      rcp(tmp * cosi*cosi + 2.0f*eta*cosi + Col3f(one));
    Col3f Rper = (tmp - 2.0f*eta*cosi + Col3f(cosi*cosi)) *
      rcp(tmp + 2.0f*eta*cosi + Col3f(cosi*cosi));
