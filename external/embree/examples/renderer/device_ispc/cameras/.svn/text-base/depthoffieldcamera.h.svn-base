...
#include "api/parms.h"
#include "depthoffieldcamera_ispc.h"
...
namespace embree
{
  struct DepthOfFieldCamera
  {
    static void* create(const Parms& parms)
    {
      ...
      const float lensRadius          = parms.getFloat("lensRadius",0.0f);
      const float focalDistance       = parms.getFloat("focalDistance");
      return  ispc::DepthOfFieldCamera__new((ispc::vec3f&)local2world.l.vx,
                                            (ispc::vec3f&)local2world.l.vy,
                                            (ispc::vec3f&)local2world.l.vz,
                                            (ispc::vec3f&)local2world.p,
                                            fov,aspectRatio,lensRadius,focalDistance);
    }
  };
}
