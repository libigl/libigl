...
#ifndef __EMBREE_ISPC_PARMS_H__
#define __EMBREE_ISPC_PARMS_H__
...
namespace embree
{
  ...
  public:
    ...
    ISPCRef getImage(const char* name, void* def = NULL) const {
      std::map<std::string,Variant>::const_iterator i = m.find(name);
    ...
    ISPCRef getTexture(const char* name, void* def = NULL) const {
      std::map<std::string,Variant>::const_iterator i = m.find(name);
