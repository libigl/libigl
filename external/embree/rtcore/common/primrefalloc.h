...
namespace embree
{
  ...
  public:
    ...
    public:
      ...
      __forceinline atomic_set<PrimRefBlock>::item* malloc(const TaskScheduler::ThreadInfo& thread, AllocatorBase* alloc) 
      {
  ...
  public:
    ...
    __forceinline atomic_set<PrimRefBlock>::item* malloc(const TaskScheduler::ThreadInfo& thread) {
      return threadPrimBlockAllocator[thread.id].malloc(thread,this);
    }

    /*! Frees a primitive block */
    __forceinline void free(const TaskScheduler::ThreadInfo& thread, atomic_set<PrimRefBlock>::item* block) {
      return threadPrimBlockAllocator[thread.id].free(block);
    }
