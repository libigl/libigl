...
namespace embree
{
    ...
    class HeuristicSpatial
  {
    ...
    static std::pair<PrimRef,PrimRef> splitPrimRef(const PrimRef& ref, int dim, float pos, const BuildTriangle* triangles, const Vec3fa* vertices);
  ...
  public:
    ...
    public:
      ...
      __forceinline Mapping (const PrimInfo& pinfo)
      {
        ...
        scale = rcp(pinfo.centBounds.upper - pinfo.centBounds.lower) * Vec3f((float)num);
      ...
      __forceinline Vec3i bin(const BBox3f& box) const {
        return clamp(bin_unsafe(box),Vec3i(0),Vec3i(int(num-1)));
      }

      /*! Computes if object is on the left of a spatial split. */
      __forceinline Vec3b left(const BBox3f& box) const {
        return le_mask(box.upper,bleft);
      }

      /*! Computes if object is on the right of a spatial split. */
      __forceinline Vec3b right(const BBox3f& box) const {
        return ge_mask(box.lower,bright);
      }

      /*! for spatial split */
    public:
      Vec3f center;     //!< center for spatial splits
      Vec3f bleft;      //!< if upper bound of a primitive is smaller it goes to the left
      Vec3f bright;     //!< if lower bound of a primitive is larger it goes to the left
    ...
    public:
      ...
      __forceinline Split () 
        : objectSAH(inf), dim(0), pos(0), spatialSAH(inf), sdim(0), numFailed(0), triangles(NULL), vertices(NULL) {}
      ...
      __forceinline void split(const PrimRef& prim, PrimRef& lprim_o, PrimRef& rprim_o) const 
      {
        ...
        /*! These have to get split and put to left and right. */
          ...
          std::pair<PrimRef,PrimRef> pair = splitPrimRef(prim,sdim,mapping.center[sdim],triangles,vertices);
    ...
    public:
      ...
      const Vec3fa* vertices;          //!< array of vertices
    };
    ...
    __forceinline HeuristicSpatial(const PrimInfo& pinfo, const BuildTriangle* triangles, const Vec3fa* vertices)
      : pinfo(pinfo), mapping(pinfo), triangles(triangles), vertices(vertices) { clear(); }
  ...
  public:
    ssei lcounts, rcounts;        //< left and right count for spatial split
