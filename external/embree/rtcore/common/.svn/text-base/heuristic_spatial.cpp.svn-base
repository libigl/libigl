...
namespace embree
{
  ...
  void HeuristicSpatial<logBlockSize>::bin(const PrimRef* prims, size_t num)
  {
    for (size_t i=0; i<num; i++)
    {
      ...
      const BBox3f prim = prims[i].bounds(); const Vec3i bin = mapping.bin(prim); const Vec3f center = Vec3f(center2(prim));
      ...
      const Vec3b left  = mapping.left (prim);
      const Vec3b right = mapping.right(prim);
      ...
      for (int maxDim=0; maxDim<3; maxDim++)
      {
        ...
        /*! These have to get split and put to left and right. */
          ...
          std::pair<PrimRef,PrimRef> pair = splitPrimRef(prims[i],maxDim,mapping.center[maxDim],triangles,vertices);
