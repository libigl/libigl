...
namespace embree
{
  ...
  Splitter<Heuristic>::Splitter (size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
                                 atomic_set<PrimRefBlock>& prims, const PrimInfo& pinfo, const Split& psplit)
  {
    ...
    if (unlikely(psplit.linfo.size() == 0 || psplit.rinfo.size() == 0)) {
      FallBackSplitter<Heuristic>::split(thread,alloc,geom,prims,pinfo,lprims,linfo,lsplit,rprims,rinfo,rsplit);
    }
  ...
  template<typename Heuristic>
  void Splitter<Heuristic>::split(size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
  ...
  template<typename Heuristic>
  void Splitter<Heuristic>::split_spatial(size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
                                          atomic_set<PrimRefBlock>& prims, const PrimInfo& pinfo, const Split& split)
  {
    Heuristic lheuristic(split.linfo,geom);
    Heuristic rheuristic(split.rinfo,geom);
