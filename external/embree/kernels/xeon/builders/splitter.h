...
namespace embree
{
  ...
  public:
    ...
    Splitter (size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
              atomic_set<PrimRefBlock>& prims, const PrimInfo& pinfo, const Split& psplit);

    /*! perform specified split */
    void split(size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
               atomic_set<PrimRefBlock>& prims, const PrimInfo& pinfo, const Split& split);

    /*! perform specified spatial split */
    void split_spatial(size_t thread, PrimRefAlloc* alloc, const BuildSource* geom,
                       atomic_set<PrimRefBlock>& prims, const PrimInfo& pinfo, const Split& split);
