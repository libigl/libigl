...
namespace embree
{
  ...
  MultiThreadedSplitter<Heuristic>::MultiThreadedSplitter(size_t threadIndex, size_t threadCount, TaskScheduler::Event* event, 
                                                          PrimRefAlloc* alloc, const BuildSource* geom,
                                                          atomic_set<PrimRefBlock>& prims_i, const PrimInfo& pinfo, const Split& split, 
    ...
    : alloc(alloc), prims(prims_i), pinfo(pinfo), split(split), geom(geom), cfun(cfun), cptr(cptr)
  {
    ...
    if (unlikely(split.linfo.size() == 0 || split.rinfo.size() == 0)) {
      FallBackSplitter<Heuristic>::split(threadIndex,alloc,geom,prims,pinfo,lprims,linfo,lsplit,rprims,rinfo,rsplit);
  ...
  template<typename Heuristic>
  void MultiThreadedSplitter<Heuristic>::task_split_parallel(size_t threadIndex, size_t threadCount, 
  ...
  template<typename Heuristic>
  void MultiThreadedSplitter<Heuristic>::task_split_parallel_spatial(size_t threadIndex, size_t threadCount, 
                                                                     size_t taskIndex, size_t taskCount,
                                                                     TaskScheduler::Event* event)
  {
    Heuristic& lheuristic = lheuristics[taskIndex]; new (&lheuristic) Heuristic (split.linfo,geom);
    ...
    while (atomic_set<PrimRefBlock>::item* block = prims.take()) 
    {
      for (size_t i=0; i<block->size(); i++) 
      {
        ...
        if (lprim.geomID() != size_t(-1) && !lblock->insert(lprim)) 
        {
          lheuristic.bin(lblock->base(),lblock->size());
          lblock = lprims.insert(alloc->malloc(threadIndex));
          lblock->insert(lprim);
        ...
        if (rprim.geomID() != size_t(-1) && !rblock->insert(rprim)) 
        {
          rheuristic.bin(rblock->base(),rblock->size());
          rblock = rprims.insert(alloc->malloc(threadIndex));
          rblock->insert(rprim);
  ...
  template<typename Heuristic>
  void MultiThreadedSplitter<Heuristic>::task_split_parallel_reduce(size_t threadIndex, size_t threadCount, TaskScheduler::Event* event)
  {
  ...
  INSTANTIATE_TEMPLATE_BY_HEURISTIC(MultiThreadedSplitter);
}
