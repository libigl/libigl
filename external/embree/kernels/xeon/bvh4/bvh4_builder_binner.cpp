...
namespace embree
{
  ...
  {
    template<int BINS>
    void Binner2<BINS>::bin(const BuildRef * __restrict__ const prims,
                            const size_t begin,
                            const size_t end,
    ...
    template<int BINS>
    void Binner2<BINS>::bin_copy(const BuildRef* __restrict__ const prims,
                                 const size_t begin,
                                 const size_t end,
                                 const Mapping2<BINS>& mapping,
                                 BuildRef* __restrict__ const dest)
    {
      reset();
      if (end-begin == 0) return;
    ...
    template<int BINS>
    void Binner2<BINS>::reduce(const Binner2 binners[], size_t num, Binner2& binner_o)
    {
      binner_o = binners[0];
    ...
    template<int BINS>
    void Binner2<BINS>::best(Split2& split, const Mapping2<BINS>& mapping)
    {
      ssef rAreas[BINS];
      ssei rCounts[BINS];
    ...
    template<int BINS>
    void Binner2<BINS>::partition(BuildRef *__restrict__ const prims,
                                  const size_t begin,
                                  const size_t end,
                                  const Split2& split,
                                  const Mapping2<BINS>& mapping,
                                  ...
                                  BuildRecord& right)
    {
      ...
      assert(begin <= end);
      BuildRef* l = prims + begin;
      BuildRef* r = prims + end - 1;
      
      const float c = mapping.ofs[split.dim];
      const float s = mapping.scale[split.dim];
      const int bestSplit2Dim = split.dim;
      const int bestSplit2 = split.pos;
      ...
      while(1)
      {
        while (likely(l <= r && lt_split(l,bestSplit2Dim,c,s,bestSplit2))) {
        ...
        while (likely(l <= r && ge_split(r,bestSplit2Dim,c,s,bestSplit2))) {
    ...
    template<int BINS>
    void ParallelBinner2<BINS>::task_parallelBinning(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
    {
      Binner2<16>& bin16 = this->global_bin16[taskIndex];
    ...
    template<int BINS>
    void ParallelBinner2<BINS>::bin(BuildRecord& current, const BuildRef* src, BuildRef* dst, const size_t threadIndex, const size_t threadCount) 
    {
      rec = current;
      mapping = Mapping2<BINS>(current.bounds);
      left.reset();
      right.reset();
      this->src = src;
      this->dst = dst;
    ...
    template<int BINS>
    void ParallelBinner2<BINS>::best(Split2& split) {
      bin16.best(split,mapping);
    }
    
    template<int BINS>
    void ParallelBinner2<BINS>::task_parallelPartition(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
    {
      const size_t startID = this->rec.begin + (taskIndex+0)*this->rec.items()/taskCount;
      const size_t endID   = this->rec.begin + (taskIndex+1)*this->rec.items()/taskCount;
      ...
      BuildRef* __restrict__ src = (BuildRef*)this->src;
      BuildRef* __restrict__ dstLeft = dst + this->rec.begin + startLeft;
      BuildRef* __restrict__ dstRight = dst + this->rec.begin + startRight + this->split.numLeft;
      
      size_t lLeft = this->rec.begin + startLeft, lRight = lLeft+localNumLeft;
      size_t rLeft = this->rec.begin + startRight + this->split.numLeft, rRight = rLeft+localNumRight;
    ...
    template<int BINS>
    void ParallelBinner2<BINS>::partition(const BuildRef* src, BuildRef* dst, 
                                          Split2& split_i, 
    ...
    template class Binner2<16>;
    template class ParallelBinner2<16>;
  }
}
