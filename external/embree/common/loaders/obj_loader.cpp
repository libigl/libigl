...
namespace embree
{
  ...
  public:
    ...
    std::vector<Device::RTPrimitive> model;

    /*! Constructor. */
    OBJLoader(const FileName& fileName, Device *device);
  ...
  private:
    ...
    /*! Material handling. */
    Device::RTMaterial curMaterial;
    std::map<std::string, Device::RTMaterial> material;
  ...
  OBJLoader::OBJLoader(const FileName &fileName, Device *device) : path(fileName.path()), g_device(device)
  {
    /* open file */
    std::ifstream cin;
    ...
    Device::RTMaterial defaultMaterial = g_device->rtNewMaterial("matte");
  ...
  void OBJLoader::loadMTL(const FileName &fileName)
  {
    ...
    while (cin.peek() != -1)
    {
      ...
      if (!strncmp(token, "Ka", 2)) { parseSep(token += 2);  Vec3f value = getVec3f(token);  g_device->rtSetFloat3(cur, "Ka", value.x, value.y, value.z);  continue; }
      if (!strncmp(token, "Kd", 2)) { parseSep(token += 2);  Vec3f value = getVec3f(token);  g_device->rtSetFloat3(cur, "Kd", value.x, value.y, value.z);  continue; }
      if (!strncmp(token, "Ks", 2)) { parseSep(token += 2);  Vec3f value = getVec3f(token);  g_device->rtSetFloat3(cur, "Ks", value.x, value.y, value.z);  continue; }
      if (!strncmp(token, "Tf", 2)) { parseSep(token += 2);  Vec3f value = getVec3f(token);  g_device->rtSetFloat3(cur, "Tf", value.x, value.y, value.z);  continue; }

      if (!strncmp(token, "map_d" , 5)) { parseSepOpt(token += 5);  g_device->rtSetTexture(cur, "map_d" , loadTexture(path + std::string(token), g_device));  continue; }
      if (!strncmp(token, "map_Ns", 6)) { parseSepOpt(token += 6);  g_device->rtSetTexture(cur, "map_Ns", loadTexture(path + std::string(token), g_device));  continue; }
      if (!strncmp(token, "map_Ka", 6)) { parseSepOpt(token += 6);  g_device->rtSetTexture(cur, "map_Ka", loadTexture(path + std::string(token), g_device));  continue; }
      if (!strncmp(token, "map_Kd", 6)) { parseSepOpt(token += 6);  g_device->rtSetTexture(cur, "map_Kd", loadTexture(path + std::string(token), g_device));  continue; }
      if (!strncmp(token, "map_Ks", 6)) { parseSepOpt(token += 6);  g_device->rtSetTexture(cur, "map_Ks", loadTexture(path + std::string(token), g_device));  continue; }

      /*! the following are extensions to the standard */
      if (!strncmp(token, "map_Refl", 8)) { parseSepOpt(token += 8);  g_device->rtSetTexture(cur, "map_Refl", loadTexture(path + std::string(token), g_device));  continue; }
      if (!strncmp(token, "map_Bump", 8)) { parseSepOpt(token += 8);  g_device->rtSetTexture(cur, "map_Bump", loadTexture(path + std::string(token), g_device));  continue; }
  ...
  void OBJLoader::flushFaceGroup()
  {
    ...
    Device::RTData dataPositions = g_device->rtNewData("immutable", positions.size() * sizeof(Vec3f), (positions.size() ? &positions[0] : NULL));
    Device::RTData dataTriangles = g_device->rtNewData("immutable", triangles.size() * sizeof(Vec3i), (triangles.size() ? &triangles[0] : NULL));
    ...
    if (normals.size()  ) {
      Device::RTData dataNormals = g_device->rtNewData("immutable", normals.size() * sizeof(Vec3f), (normals.size() ? &normals[0] : NULL));
      g_device->rtSetArray(mesh, "normals", "float3", dataNormals, normals.size(), sizeof(Vec3f), 0);
    }
    if (texcoords.size()) {
      Device::RTData dataTexCoords = g_device->rtNewData("immutable", texcoords.size() * sizeof(Vec2f), (texcoords.size() ? &texcoords[0] : NULL));
  ...
  std::vector<Device::RTPrimitive> loadOBJ(const FileName &fileName, Device *device) {
    OBJLoader loader(fileName, device); return loader.model;
  }
}
