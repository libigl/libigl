...
#include <igl/STR.h>
template <> struct Catch::StringMaker<std::tuple<int, int, int>> {
  static std::string convert(std::tuple<int, int, int> const &t) {
    return STR("(" << std::get<0>(t) << "," << std::get<1>(t) << ","
                   << std::get<2>(t) << ")");
  }
};
template <> struct Catch::StringMaker<std::tuple<int, int, double>> {
...
void assert_eq(const Eigen::MatrixBase<DerivedA> &A,
...
template <typename DerivedA, typename DerivedB>
void assert_neq(const Eigen::MatrixBase<DerivedA> &A,
                const Eigen::MatrixBase<DerivedB> &B) {
  // Sizes should match
  REQUIRE(A.rows() == B.rows());
  REQUIRE(A.cols() == B.cols());
...
template <typename DerivedA, typename DerivedB>
void assert_eq(const Eigen::SparseMatrix<DerivedA> &A,
...
void assert_near(const Eigen::MatrixBase<DerivedA> &A,
                 const Eigen::MatrixBase<DerivedB> &B, const EpsType &eps) {
  // Sizes should match
  REQUIRE(A.rows() == B.rows());
  REQUIRE(A.cols() == B.cols());
  for (int i = 0; i < A.rows(); i++) {
